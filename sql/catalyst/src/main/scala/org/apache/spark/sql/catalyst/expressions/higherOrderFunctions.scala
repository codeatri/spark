...
case class TransformValues(
  ...
  override def dataType: DataType = {
    val keyType = input.dataType.asInstanceOf[MapType].keyType
    MapType(keyType, function.dataType, input.nullable)
  }
  ...
  override def eval(input: InternalRow): Any = {
    ...
    if (arr == null) {
      ...
      val f = functionForEval
      val resultValues = new GenericArrayData(new Array[Any](arr.numElements))
      var i = 0
      while (i < arr.numElements) {
        keyVar.value.set(arr.keyArray().get(i, keyVar.dataType))
        valueVar.value.set(arr.valueArray().get(i, valueVar.dataType))
        resultValues.update(i, f.eval(input))
        i += 1
      }
      new ArrayBasedMapData(arr.keyArray(), resultValues)
    }
  }
